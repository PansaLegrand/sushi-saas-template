...
4. ~~**No committed retention period for the audit tables.**~~ **Fixed.** The
   ...
   provider receipts, 90 days for auth events, and 365 days for admin audit
   logs. The first two are maintained automatically; the audit datasets require
   an explicit dry-run and confirmed apply. See
   [backups-and-retention.md](backups-and-retention.md). Product and financial
   tables are outside this generic policy.
