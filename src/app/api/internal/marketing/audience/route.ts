...
import { MarketingAudienceRequestSchema } from "@/services/marketing/contracts";
import { previewMarketingAudience } from "@/services/marketing/operations";
...
export async function POST(req: Request) {
  try {
    ...
    const parsed = MarketingAudienceRequestSchema.safeParse(json);
    if (!parsed.success) {
    ...
    return respData(await previewMarketingAudience(parsed.data));
  } catch (error) {
    return respError(error, {
      fallback: "SERVER_ERROR",
      logFields: { event: "marketing.audience_preview_failed" },
    });
  }
}
