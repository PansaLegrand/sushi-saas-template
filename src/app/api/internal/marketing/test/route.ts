...
import { MarketingTestRequestSchema } from "@/services/marketing/contracts";
import { queueMarketingTest } from "@/services/marketing/dispatch";
...
export async function POST(req: Request) {
  try {
    ...
    const parsed = MarketingTestRequestSchema.safeParse(json);
    if (!parsed.success) {
    ...
    return respData(await queueMarketingTest(parsed.data));
  } catch (error) {
    return respError(error, {
      fallback: "SERVER_ERROR",
      logFields: { event: "marketing.test_queue_failed" },
    });
  }
}
