...
import { cancelMarketingCampaign } from "@/services/marketing/operations";
...
export async function POST(req: Request) {
  try {
    ...
    return respData(await cancelMarketingCampaign(parsed.data));
  } catch (error) {
    return respError(error, {
      fallback: "SERVER_ERROR",
      logFields: { event: "marketing.cancel_failed" },
    });
  }
}
