...
Ajustez les horaires, la durée de rétention et le fuseau horaire dans `src/config/reservations.ts`.
...
- Fuseau horaire : modifiez `baseTimeZone` dans `src/config/reservations.ts` (ex. `Europe/Paris`).
