...
icon: "Database"
description: Préparer Postgres et Drizzle ORM pour Better Auth et les tables de l’application.
keywords:
  - Postgres
  - Drizzle ORM
  - migrations
...
Avant d’utiliser les nouvelles pages d’authentification, il faut une base de
données prête. Ce guide détaille l’ORM livré (Drizzle), les scénarios possibles,
les fichiers à modifier et les commandes à lancer ensuite.
...
- `src/db/schema.ts` – modèles Drizzle pour **Better Auth** (`users`, `sessions`,
  `accounts`, `verifications`) et les tables métier (`orders`, `credits`, etc.).
