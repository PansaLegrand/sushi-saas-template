...
canonical: "https://www.sushi-templates.com/fr/blogs/stripe-setup"
---
...
- `/api/checkout` (POST) : résout le produit dans `src/config/billing.ts`, crée la session avec son Price ID, puis enregistre une `order` `created`. Requiert `Idempotency-Key` : rejouer la même clé pour la même organisation renvoie la commande/session existante ; une nouvelle clé démarre un autre achat et peut créer un autre abonnement.
...
5) Vérifiez avec `POST /api/account/credits`. Pour utiliser le playground local, activez `ENABLE_DEMO_FEATURES=true` et `ENABLE_CREDITS_PLAYGROUND=true`.
...
- Les conditions commerciales vivent dans `src/config/billing.ts` ; `src/config/pricing.ts` en dérive les cartes visuelles.
...
STRIPE_PRICE_PLUS_MONTHLY_CNY=price_...
STRIPE_PRICE_PLUS_YEARLY_CNY=price_...
STRIPE_PRICE_MAX_MONTHLY_CNY=price_...
STRIPE_PRICE_MAX_YEARLY_CNY=price_...
```

Comportement des IDs de Prix

- Checkout utilise toujours un Price ID configuré ; aucun prix récurrent inline n’est créé.
- Le démarrage en production échoue si un Price ID USD requis manque ou est mal formé.
- Les anciennes variables `NEXT_PUBLIC_*` restent prises en charge pour les abonnements existants.
...
- Idempotence : les clés dérivées de la période sécurisent le rejeu et réparent un crédit manquant.
