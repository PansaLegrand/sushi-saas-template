...
- `/api/checkout` (POST) : valide le plan (`src/config/pricing.ts`), crée la session, enregistre une `order` `created`.
...
- Sinon, Checkout crée la session avec `price_data` inline depuis `src/config/pricing.ts` (repli sûr).
