...
Les checks de rôle sont dans `apps/admin/lib/authz.ts` :

- `requireAdminRead()` -> autorise `admin_ro` et `admin_rw`
- `requireAdminWrite()` -> autorise `admin_rw`

Garde de layout admin : `apps/admin/app/(admin)/layout.tsx`.
...
- Dashboard : `apps/admin/app/(admin)/page.tsx`
- Feedbacks : `apps/admin/app/(admin)/feedbacks/page.tsx`
- Réservations : `apps/admin/app/(admin)/reservations/page.tsx`
- Affiliés : `apps/admin/app/(admin)/affiliates/page.tsx`
...
- `GET /api/admin/orders` -> commandes payées paginées
...
L'URL Redis est obligatoire en production en mode app. Utilisez `redis://` en
local et l'URL `rediss://` fournie par votre service Redis managé en production.
Le développement local peut l'omettre et utiliser le limiteur en mémoire
mono-processus. Les limites d'authentification sont séparées selon le risque :
inscription, 5 toutes les 15 minutes ; connexion, 10 par minute ; e-mails de
récupération/vérification, 5 toutes les 15 minutes ; changements sensibles de
mot de passe, d'e-mail, de session et de 2FA, 10 toutes les 5 minutes. Ces
limites complètent Turnstile ; elles ne le remplacent pas.
