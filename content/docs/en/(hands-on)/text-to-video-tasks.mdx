...
canonical: "https://www.sushi-templates.com/docs/text-to-video-tasks"
---
...
- Configure pricing in `src/config/tasks.ts`:
...
- Dev mock provider: set `ENABLE_DEMO_FEATURES=true`, `ENABLE_TEXT2VIDEO_MOCK=true`, and optionally `TEXT2VIDEO_MOCK_URL` in `.env.local`.
...
Open at `/tasks/text-to-video`. If you are not signed in, the page will prompt you to go to the sign‑up page.
