...
canonical: "https://www.sushi-templates.com/docs/stripe-setup"
---
...
- `/api/checkout` (POST):
  - Resolves the selected product against `src/config/billing.ts`.
  - Creates a Stripe Checkout session with its configured Stripe Price ID.
  - Inserts an `orders` row with status `created` and returns `checkout_url`.
  - Requires an `Idempotency-Key`. Replaying the same key for the same organization returns the existing order/session; a new key deliberately starts another purchase and may create another subscription.
...
5) Verify credits with `POST /api/account/credits`. For the local playground route, enable `ENABLE_DEMO_FEATURES=true` and `ENABLE_CREDITS_PLAYGROUND=true`.
...
- Commercial terms live in `src/config/billing.ts`; `src/config/pricing.ts` derives display cards from that catalog.
...
- Checkout always uses a configured Stripe Price ID; there is no inline recurring-price fallback.
- Production startup fails when a required USD Price ID is missing or malformed.
- Legacy `NEXT_PUBLIC_*` variables remain supported for grandfathered subscriptions.
...
- Idempotency: the period-derived order and ledger keys make replay safe and repair a missing grant.
