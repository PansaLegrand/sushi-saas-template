...
- `/api/checkout` (POST):
  - Validates the selected plan against `src/config/pricing.ts`.
...
- Plans live in `src/config/pricing.ts`. Values are typed (`Pricing`), shared by the pricing page and `/api/checkout`.
...
- If not, Checkout creates the session with inline `price_data` using amounts from `src/config/pricing.ts` (safe fallback).
