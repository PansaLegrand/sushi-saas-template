...
canonical: "https://www.sushi-templates.com/docs/reservations-feature"
---
...
Visit `/reserve` to try the flow. A demo service is auto‑seeded by default. Tweak hours/timezone in `src/config/reservations.ts`.
...
5. Review bookings at `/account/reservations`

## Customization

- Base timezone: edit `baseTimeZone` in `src/config/reservations.ts` (e.g., `Europe/Paris`). Availability uses this tz; the UI displays in the user’s local tz.
