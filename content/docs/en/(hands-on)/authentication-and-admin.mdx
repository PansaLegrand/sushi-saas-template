...
canonical: "https://www.sushi-templates.com/docs/authentication-and-admin"
---
...
Role checks are implemented in `apps/admin/lib/authz.ts`:

- `requireAdminRead()` -> allows `admin_ro` and `admin_rw`
- `requireAdminWrite()` -> allows `admin_rw`

Admin layout guard: `apps/admin/app/(admin)/layout.tsx`.
...
- Reservations: `apps/admin/app/(admin)/reservations/page.tsx`
- Affiliates: `apps/admin/app/(admin)/affiliates/page.tsx`
...
- `GET /api/admin/orders` -> paginated paid orders
...
The Redis URL is mandatory in production app mode. Use `redis://` locally and
the `rediss://` URL supplied by your managed Redis provider in production.
Local development may omit it and use the single-process memory limiter. Auth limits are separated
by risk: signup is 5 per 15 minutes, sign-in is 10 per minute,
recovery/verification email is 5 per 15 minutes, and sensitive password,
email, session, and 2FA changes are 10 per 5 minutes. These limits complement
Turnstile; they do not replace it.
