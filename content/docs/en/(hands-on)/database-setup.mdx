...
icon: "Database"
description: Configure Postgres and Drizzle ORM — the backbone for auth, billing, storage, tasks, and app tables.
keywords:
  - Postgres
  - Drizzle ORM
  - migrations
...
canonical: "https://www.sushi-templates.com/docs/database-setup"
---
...
- When adding a feature: model tables in `src/db/schema.ts` → `pnpm drizzle-kit generate` → `pnpm drizzle-kit migrate` → add `src/models/<domain>.ts` → call from `src/services/*` and routes.
...
Once migrations are applied, restart `pnpm dev`, open `/signup`, and create a
