...
canonical: "https://www.sushi-templates.com/docs/quick-start"
---
...
- Docs (MDX): `/docs/quick-start` or `/:locale/docs/quick-start`
...
- Bundled locales are declared in `src/i18n/locale.ts`; enabled locales are controlled by `NEXT_PUBLIC_LOCALES`.
- Use `NEXT_PUBLIC_LOCALES=en` for English only, or `NEXT_PUBLIC_LOCALES=en,es` for English and Spanish.
...
2) Add the code to `availableLocales` in `src/i18n/locale.ts`
3) Include the code in `NEXT_PUBLIC_LOCALES`
4) Restart dev
...
`/docs/<slugs>` for English and `/:locale/docs/<slugs>` for other languages.
...
- If a locale route 404s, add that code to `NEXT_PUBLIC_LOCALES` and restart.
