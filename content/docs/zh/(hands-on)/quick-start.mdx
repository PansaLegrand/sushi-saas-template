...
- 文案在 `messages/*.json`
- 内置语言在 `src/i18n/locale.ts`；启用哪些语言由 `NEXT_PUBLIC_LOCALES` 控制
- 只启用英文：`NEXT_PUBLIC_LOCALES=en`；启用英文和西班牙语：`NEXT_PUBLIC_LOCALES=en,es`
...
2）在 `src/i18n/locale.ts` 的 `availableLocales` 中加入该语言
3）把该语言加入 `NEXT_PUBLIC_LOCALES`
4）重启开发服务
...
- 语言路由 404：把该语言代码加入 `NEXT_PUBLIC_LOCALES` 并重启
