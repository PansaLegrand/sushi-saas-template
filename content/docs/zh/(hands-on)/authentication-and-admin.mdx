...
角色校验在 `apps/admin/lib/authz.ts`：

- `requireAdminRead()` -> 允许 `admin_ro` 与 `admin_rw`
- `requireAdminWrite()` -> 仅允许 `admin_rw`

Admin 布局守卫：`apps/admin/app/(admin)/layout.tsx`。
...
- 仪表盘：`apps/admin/app/(admin)/page.tsx`
- 反馈：`apps/admin/app/(admin)/feedbacks/page.tsx`
- 预约：`apps/admin/app/(admin)/reservations/page.tsx`
- 分销：`apps/admin/app/(admin)/affiliates/page.tsx`

### Admin API 路由

- `GET /api/admin/users/[uuid]/credits` -> 积分汇总 + 台账
- `POST /api/admin/credits/grant` -> 发放积分
- `GET /api/admin/orders` -> 分页已支付订单
...
生产 app 模式必须配置 Redis URL。本地使用 `redis://`，生产环境使用
托管 Redis 服务提供的 `rediss://` URL。本地开发可以省略，并使用单进程
内存限流器。认证限流按风险拆分：注册为每15分钟5次，
登录为每分钟10次，找回/验证邮件为每15分钟5次，密码、邮箱、会话及
2FA 等敏感变更为每5分钟10次。这些限制用于补充 Turnstile，而不是
替代它。
