...
- `/api/checkout` (POST)：从 `src/config/billing.ts` 解析商品，使用已配置的 Price ID 创建 Checkout 会话，并保存订单 `created`。必须提供 `Idempotency-Key`：同一组织重复提交同一 key 会返回已有订单/会话；使用新 key 则会发起另一笔购买，并可创建额外订阅。
...
5) 使用 `POST /api/account/credits` 验证余额变化。如需本地 playground，请设置 `ENABLE_DEMO_FEATURES=true` 和 `ENABLE_CREDITS_PLAYGROUND=true`。
...
- 商业条款集中在 `src/config/billing.ts`；`src/config/pricing.ts` 从中生成展示卡片。
...
- Checkout 始终使用已配置的 Price ID，不再创建内联周期价格。
- 必需的 USD Price ID 缺失或格式错误时，生产服务会在启动时失败。
- 旧的 `NEXT_PUBLIC_*` 变量仍用于兼容已有订阅。
...
- 幂等：由账期派生的订单和账本键保证重放安全，并可修复缺失的积分发放。
