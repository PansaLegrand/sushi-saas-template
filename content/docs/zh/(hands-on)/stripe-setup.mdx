...
- `/api/checkout` (POST)：校验 `src/config/pricing.ts` 的计划，创建 Checkout 会话，保存订单 `created`。
...
- 套餐定义：`src/config/pricing.ts`（强类型、前后端共用）。
...
- 若未配置，Checkout 会使用 `src/config/pricing.ts` 的金额通过内联 `price_data` 创建会话（安全回退）。
