...
- 文言: `messages/*.json`
- 同梱言語は `src/i18n/locale.ts`、有効化する言語は `NEXT_PUBLIC_LOCALES` で設定
- 英語のみ: `NEXT_PUBLIC_LOCALES=en`、英語とスペイン語: `NEXT_PUBLIC_LOCALES=en,es`
...
2) `src/i18n/locale.ts` の `availableLocales` に言語コードを追加
3) `NEXT_PUBLIC_LOCALES` にそのコードを含める
4) サーバーを再起動
...
- 言語ルートが 404 になる場合: `NEXT_PUBLIC_LOCALES` にそのコードを追加して再起動
