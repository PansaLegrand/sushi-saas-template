...
ロールチェック実装: `apps/admin/lib/authz.ts`

- `requireAdminRead()` -> `admin_ro` と `admin_rw` を許可
- `requireAdminWrite()` -> `admin_rw` のみ許可

管理画面レイアウトガード: `apps/admin/app/(admin)/layout.tsx`
...
- ダッシュボード: `apps/admin/app/(admin)/page.tsx`
- フィードバック: `apps/admin/app/(admin)/feedbacks/page.tsx`
- 予約: `apps/admin/app/(admin)/reservations/page.tsx`
- アフィリエイト: `apps/admin/app/(admin)/affiliates/page.tsx`

### Admin API ルート

- `GET /api/admin/users/[uuid]/credits` -> クレジット概要 + 台帳
- `POST /api/admin/credits/grant` -> クレジット付与
- `GET /api/admin/orders` -> ページング済みの支払い済み注文一覧
...
本番の app モードでは Redis URL が必須です。ローカルでは `redis://`、
本番ではマネージド Redis が提供する `rediss://` URL を使います。
ローカル開発では省略し、単一プロセスのメモリ内リミッターを利用できます。
認証の制限はリスク別です。サインアップは15分に5回、サインインは
1分に10回、復旧/確認メールは15分に5回、パスワード・メール・
セッション・2FA の重要な変更は5分に10回です。これらは Turnstile を
補完するもので、置き換えるものではありません。
