...
canonical: "https://www.sushi-templates.com/ja/blogs/reservations-feature"
---
...
営業時間・ホールド時間・基準タイムゾーンは `src/config/reservations.ts` を編集してください。
...
- 基準タイムゾーン：`src/config/reservations.ts` の `baseTimeZone` を編集（例：`Asia/Tokyo`）
