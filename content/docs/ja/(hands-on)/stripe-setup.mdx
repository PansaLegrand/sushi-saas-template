...
canonical: "https://www.sushi-templates.com/ja/blogs/stripe-setup"
---
...
- `/api/checkout` (POST): `src/config/billing.ts` から商品を解決し、設定済み Price ID で Checkout セッションを作成、注文を `created` で保存。`Idempotency-Key` は必須です。同じ組織で同じキーを再送すると既存の注文/セッションを返し、新しいキーなら別の購入として追加のサブスクリプションを作成できます。
...
5) `POST /api/account/credits` で残高を確認します。ローカルの playground を使う場合は `ENABLE_DEMO_FEATURES=true` と `ENABLE_CREDITS_PLAYGROUND=true` を設定します。
...
- 商用条件は `src/config/billing.ts` に集約し、`src/config/pricing.ts` は表示カードをそこから生成します。
...
- Checkout は必ず設定済み Price ID を使用し、定期価格のインラインフォールバックは行いません。
- 必須の USD Price ID が未設定または不正な場合、本番サーバーは起動時に失敗します。
- 既存サブスクリプション向けに旧 `NEXT_PUBLIC_*` 変数も引き続き対応します。
...
- 冪等性：期間から導出した注文・台帳キーにより再送は安全で、不足した付与も修復できます。
