...
- `/api/checkout` (POST): `src/config/pricing.ts` を元に検証し、Checkout セッションを作成、注文を `created` で保存。
...
- プラン: `src/config/pricing.ts` に定義（型安全）。
...
- 無い場合は、`src/config/pricing.ts` の金額でインライン `price_data` を使ってセッションを作成します（安全なフォールバック）。
