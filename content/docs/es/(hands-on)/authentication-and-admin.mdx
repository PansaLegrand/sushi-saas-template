...
Los checks de rol están en `apps/admin/lib/authz.ts`:

- `requireAdminRead()` -> permite `admin_ro` y `admin_rw`
- `requireAdminWrite()` -> permite `admin_rw`

Guard de layout admin: `apps/admin/app/(admin)/layout.tsx`.
...
- Reservas: `apps/admin/app/(admin)/reservations/page.tsx`
- Afiliados: `apps/admin/app/(admin)/affiliates/page.tsx`
...
- `GET /api/admin/orders` -> pedidos pagados paginados
...
La URL de Redis es obligatoria en producción en modo app. Usa `redis://`
localmente y la URL `rediss://` de tu proveedor de Redis administrado en
producción. En desarrollo local se puede omitir para usar el limitador en memoria de un
solo proceso. Los límites de auth se separan según el riesgo: registro, 5 cada
15 minutos; inicio de sesión, 10 por minuto; correo de recuperación/verificación,
5 cada 15 minutos; y cambios sensibles de contraseña, email, sesión y 2FA, 10
cada 5 minutos. Estos límites complementan Turnstile; no lo sustituyen.
