...
- `/api/checkout` (POST): valida el plan en `src/config/pricing.ts`, crea la sesión y guarda un `order` con estado `created`.
...
- Planes en `src/config/pricing.ts` (tipados y compartidos).
...
- Si no, Checkout crea la sesión con `price_data` inline usando los importes de `src/config/pricing.ts` (fallback seguro).
