...
- `/api/checkout` (POST): resuelve el producto en `src/config/billing.ts`, crea la sesión con su Price ID y guarda un `order` con estado `created`. Requiere `Idempotency-Key`: repetir la misma clave en la misma organización devuelve el pedido/sesión existente; una clave nueva inicia otra compra y puede crear otra suscripción.
...
5) Verifica con `POST /api/account/credits`. Para usar el playground local, habilita `ENABLE_DEMO_FEATURES=true` y `ENABLE_CREDITS_PLAYGROUND=true`.
...
- Los términos comerciales viven en `src/config/billing.ts`; `src/config/pricing.ts` deriva las tarjetas visuales.
...
Comportamiento de los IDs de Precio

- Checkout siempre usa un Price ID configurado; no existe fallback de precio recurrente inline.
- El arranque en producción falla si falta un Price ID USD obligatorio o su formato no es válido.
- Las variables antiguas `NEXT_PUBLIC_*` siguen admitidas para suscripciones existentes.
...
- Idempotencia: las claves derivadas del período hacen seguro el replay y reparan una concesión ausente.
