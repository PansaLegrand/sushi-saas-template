...
Este es un recorrido a nivel de paquetes para el login social usando Google como ejemplo. Verás quién inicia cada paso, qué request HTTP se envía y qué hace tu backend. El mismo patrón aplica a Apple, Facebook, GitHub y otros proveedores OAuth/OIDC.
...
- OAuth es un flujo de redirección: navegador → proveedor → callback → intercambio de tokens en backend.
- `id_token` prueba identidad; `access_token` sirve para APIs del proveedor.
- Verifica siempre tokens en servidor (`iss`, `aud`, `exp`, firma) y valida `state`.
- Tras el login emites tu propia sesión/JWT; Google sale del camino.
- El mismo flujo funciona para Apple, GitHub, Facebook, etc., con campos distintos.
