...
Ce guide explique l’authentification web de bout en bout : inscription, login et maintien de session. Il compare JWT et sessions, “remember me” vs refresh, et montre comment OAuth (Google) s’insère dans le même flux.
...
- L’auth résout toujours trois tâches : inscription, login, maintien de session.
- JWT et sessions résolvent le même problème avec des mécaniques différentes.
- Access token / session ID sont courts ; refresh / remember les renouvellent.
- OAuth ne remplace que la vérification du mot de passe ; vous émettez votre session/JWT.
- JWT pour clients distribués ; sessions pour révocation simple et sites classiques.
