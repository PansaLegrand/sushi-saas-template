...
| `config/` | Compile-time constants and product config: pricing plans, billing amounts, auth route map | Static values only — no I/O, no runtime state |
| `db/` | Drizzle schema, migrations, connection factory | Read `docs/database.md` before changing the schema |
| `features/` | Self-contained vertical slices. **Currently only `reservations/`** | See the note below before adding another |
...
`features/reservations/` is a vertical slice with its own models, service, config, and components. **It is the only one**, and the rest of the repo is horizontally layered. Do not add a second `features/` module without deciding which architecture the repo is committing to — two competing conventions is worse than either one. If you extend reservations, note that `features/reservations/models.ts` calls `db()` directly, which the layering rule above forbids everywhere else.
